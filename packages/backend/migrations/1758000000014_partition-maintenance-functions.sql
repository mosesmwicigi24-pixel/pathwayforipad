-- Migration 14 · Partition-maintenance functions (spec §2.4, §5.9)
-- ============================================================================
-- Installs the provision/prune functions (mirroring src/db/partition-maintenance.sql)
-- so the scheduled worker can call them by name. Idempotent (CREATE OR REPLACE).
...
DROP FUNCTION IF EXISTS fn_prune_interaction_partitions(INT);
DROP FUNCTION IF EXISTS fn_provision_interaction_partitions(INT);
