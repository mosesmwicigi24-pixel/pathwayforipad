...
import { ChevronRight, CircleAlert, Sparkles, TrendingUp, Users, ArrowUpRight } from "lucide-react";
import { AdminApi, type EngagementCellRow } from "../../api/client";
import { errorMessage } from "../../util/error";

const TONES = ["#16A34A", "#0B84E8", "#7C3AED", "#C89B3C", "#DC2626", "#0D9488"];
const toneOf = (id: string): string => { let h = 0; for (let i = 0; i < id.length; i++) h = (h * 31 + id.charCodeAt(i)) % TONES.length; return TONES[h] as string; };
const initials = (name: string): string => name.split(/\s+/).map((p) => p[0]).filter(Boolean).slice(0, 2).join("").toUpperCase() || "C";
  ...
  return (
    <main className="min-h-full" style={{ background: "var(--background)" }}>
      <div style={{ background: "var(--nuru-dark)", padding: "22px clamp(16px,4vw,48px) 26px" }}>
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button onClick={() => navigate("/reflection-queue")} className="flex items-center gap-2 rounded-lg px-3" style={{ height: 32, background: "var(--nuru-gold)", color: "#fff", fontSize: 12, fontWeight: 600, border: "none" }}><ArrowUpRight size={13} /> Action queue</button>
          </div>
        </div>
        <div className="mt-5">
      ...
      <section style={{ padding: "24px clamp(16px,4vw,48px) 48px" }}>
        ...
        <div className="mb-3"><div className="nuru-eyebrow nuru-eyebrow-gold" style={{ marginBottom: 4 }}>Cell roster</div><h2 className="type-section">Cells &amp; their disciplers</h2></div>

        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-8">
          {cells.map((cell) => {
            const tone = toneOf(cell.cell_group_id); const avg = pct(cell.avg_engagement);
            return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          <div className="lg:col-span-3 rounded-3xl p-6" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            ...
            <div className="flex flex-col gap-4">
              {ranked.map((cell, idx) => { const tone = toneOf(cell.cell_group_id); const avg = pct(cell.avg_engagement); return (
          ...
          <div className="lg:col-span-2 rounded-3xl p-6" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            ...
            <div className="flex flex-col gap-3">
              {[...cells].sort((a, b) => b.at_risk - a.at_risk).map((cell) => { const tone = toneOf(cell.cell_group_id); return (
