...
function QuestionView({ q, idx, accent }: { q: AdminQuestion; idx: number; accent: string }): ReactElement {
  ...
  const opts = q.q_type === "TrueFalse" ? ["True", "False"] : (q.answer_options ?? []);
  return (
    <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 16, padding: "20px 22px" }}>
      <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: "var(--muted-foreground)", marginTop: 3 }}>{q.points} {q.points === 1 ? "point" : "points"}</div>
        </div>
      </div>
      <div style={{ paddingLeft: 34, display: "flex", flexDirection: "column", gap: 10 }}>
        {(q.q_type === "MultipleChoice" || q.q_type === "TrueFalse") && opts.map((o) => (
          <label key={o} style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 14, color: "var(--foreground)", cursor: "default" }}>
            <span style={{ width: 18, height: 18, flexShrink: 0, border: "2px solid var(--border)", borderRadius: 999, background: "var(--card)" }} /> {o}
          </label>
        ))}
