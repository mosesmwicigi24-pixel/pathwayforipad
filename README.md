...
The migrations follow §2 exactly except for the points below. The first three are places where the DDL **as written would not compile or run on PostgreSQL**; the last two are minimal schema additions required to implement API behaviour (§3.3, §5.3) that the §2 DDL did not provide for. Each is fixed minimally and flagged in-file:
...
4. **SSO provisioning + profile versioning (`migration 011`, §3.3/§5.3).** `POST /v1/auth/oauth/{provider}` provisions a user at first login, but the baseline intake (`date_of_birth`, `phone_number`, and the `congregation_id` derived from the chosen cell) only arrives later at onboarding — while §2 marks them `NOT NULL`. Following the spec's own SSO treatment of `email`/`password_hash`, those three columns are made nullable; onboarding re-enforces them before creating the enrollment, so no *active* member ever lacks them. The same migration adds `users.row_version` (the §2 DDL has none) for the `PATCH /v1/me` optimistic-concurrency check, and hardens the `is_minor` trigger to treat an unknown DOB as non-minor.
5. **Admin step-up MFA (`migration 012`, §5.3).** §5.3 requires MFA for SuperAdmin / financial-config actions, but §2 defines no MFA storage. Added `users.mfa_secret` (TOTP shared secret, AES-256-GCM sealed at the app layer — never plaintext), `users.mfa_enabled`, and `users.mfa_enrolled_at`. **Trade-off:** the sealing key is currently derived from `JWT_SIGNING_KEY`; production should supply a dedicated, separately-rotated MFA key (§5.10).
6. **PRD curriculum shape + author-driven CMS (`migration 015`, §2.2/§1.9).** The engineering spec assumed **5 levels / 45 modules**; the real PRD ("DISCIPLESHIP CLASSES") has **6 levels and ~51 unevenly-distributed modules**, with scripture-prose lessons and *authored* (not source-provided) quizzes. So: levels/modules are created and edited in-app (no file seed); **curriculum size is data-driven** — the engagement Cᵢ denominator uses the live count of *published* modules (the `CURRICULUM_MODULE_COUNT` constant is now only a zero-count fallback). Modules gained a lifecycle `status` enum (`draft`/`published`/`archived`) with `is_published` kept as a generated mirror, plus `summary`/`key_verses`/`row_version`/`updated_at`. Gating is **evaluation-kind aware** (`none`/`reflection`/`quiz`/`exit_exam`). The reference-integrity check now asserts the 6 verbatim titles and per-level published-sequence contiguity instead of a magic module count.
